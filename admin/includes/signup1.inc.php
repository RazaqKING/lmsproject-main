<?php
  		if ( ! (empty($_POST['uid'])|| trim($_POST['uid'])=='') && !(empty($_POST['mail'])|| trim($_POST['mail'])=='') && !(empty($_POST['pwd'])|| trim($_POST['pwd'])=='') && !(empty($_POST['pwd-repeat'])|| trim($_POST['pwd-repeat'])=='') ) {
  			...
  			if(!filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/",$_POST['uid'])){
				      header("Location: ../register.php?errorp=invalidmailuid");
				      exit();

			    }else if (!filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL)){
				      header("Location: ../register.php?errorp=invalidmail&uid=".$_POST['uid']);
				      exit();
			    }
			    else if (! preg_match("/^[a-z A-Z0-9]*$/",$_POST['uid']) ){
				      header("Location: ../register.php?errorp=invaliduid&mail=".$_POST['mail']);
				      exit();

			    }else if( $_POST['pwd'] !== $_POST['pwd-repeat'] ) {
					  header("Location: ../register.php?errorp=passwordcheck&mail=".$_POST['mail']."&uid=".$_POST['uid']);
					  exit();
					 }
				/*Si tout va bient on va connecter au base de données et voir :1/ si le nom d'utilisateur existe deja */
				try{
					$db=new PDO('mysql:host=localhost;dbname=lacademy;charset=utf8','root','',array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
				}
				catch(Exception $err){
				...
				$req->execute(array('u'=>$_POST['uid'],'e'=>$_POST['mail']));
				...
				$req->closeCursor();
				if ($i!==0){
					header("Location: ../register.php?errorp=usertaken&mail=".$_POST['mail']);
        			exit();
				}
      ...
      if((empty($_POST['uid'])|| trim($_POST['uid'])=='') && (empty($_POST['mail'])|| trim($_POST['mail'])=='')){
        header("Location: ../register.php?errorp=emptyfields");
      }elseif(! (empty($_POST['uid'])|| trim($_POST['uid'])=='') && (empty($_POST['mail'])|| trim($_POST['mail'])=='')){
          header("Location: ../register.php?errorp=emptyfields&uid=".$_POST['uid']);
      }elseif( (empty($_POST['uid'])|| trim($_POST['uid'])=='') && ! (empty($_POST['mail'])|| trim($_POST['mail'])=='')){
        header("Location: ../register.php?errorp=emptyfields&mail=".$_POST['mail']);
      }elseif(! (empty($_POST['uid'])|| trim($_POST['uid'])=='') && !(empty($_POST['mail'])|| trim($_POST['mail'])=='')){
        header("Location: ../register.php?errorp=emptyfields&mail=".$_POST['mail']."&uid=".$_POST['uid']);
      }
  			exit();
		}
