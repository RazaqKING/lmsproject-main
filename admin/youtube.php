...
<?php 
      if(isset($_POST['submit'])){
           ...
           $courselink = $_POST['course_link'];
           if(!(empty($coursename)) && !(empty($coursedesc)) && !(empty($courselink))){
            ...
            //extract the ID
               ...
               //the ID of the YouTube URL: eLl7Y29eC7o
                  $videoid = $matches[1];
            $query = "INSERT INTO `youtube` (`course_name`, `course_desc`, `course_link`) ";
            $query .= "VALUES('{$coursename}', '{$coursedesc}', '{$videoid}')";
